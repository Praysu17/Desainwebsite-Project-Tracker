...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Proposals that have not been responded to yet (Draft = belum dikirim, Terkirim / Dilihat = belum direspon klien)
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3.5 py-5 space-y-1.5 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {/* Quick Client Proposal Link Box - Hanya tampil jika ada proposal yang belum dikirim atau belum direspon klien */}
            ...
            <div className="pt-4 mt-4 border-t border-slate-800/80">
              <div className="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400 flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  {pendingProposals.length} Belum Respon
                </span>
              </div>

              <div className="bg-slate-900/90 rounded-xl p-3 border border-slate-800 space-y-2">
                <p className="text-xs text-slate-300 font-medium leading-snug">
                  Proposal online aktif menunggu respon klien:
                </p>

                <div className="space-y-1.5">
                  {pendingProposals.slice(0, 2).map((prop) => (
                    <a
                      key={prop.id}
                      href={`/share/proposal/${prop.shareToken}`}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center justify-between w-full px-2.5 py-2 text-xs font-semibold text-cyan-300 bg-cyan-950/60 hover:bg-cyan-900/80 border border-cyan-500/40 rounded-lg transition-colors group"
                      title={`Buka Proposal: ${prop.projectTitle} (${prop.clientName})`}
                    >
                      <div className="min-w-0 pr-2 text-left">
                        <span className="block truncate font-bold text-white group-hover:text-cyan-200">
                          {prop.clientName}
                        </span>
                        <span className="text-[10px] text-slate-400 block truncate">
                          {prop.projectTitle} • {prop.status === 'Draft' ? 'Belum Dikirim' : prop.status === 'Dilihat' ? 'Sudah Dilihat' : 'Terkirim'}
                        </span>
                      </div>
                      <ExternalLink className="w-3.5 h-3.5 shrink-0 ml-1 text-cyan-400 group-hover:text-cyan-200" />
                    </a>
                  ))}
                </div>
                ...
                {pendingProposals.length > 2 && (
                  <button
                    ...
                    className="text-[11px] text-slate-400 hover:text-cyan-300 font-medium w-full text-center transition-colors block pt-1 cursor-pointer"
                  >
                    +{pendingProposals.length - 2} proposal lainnya →
                  </button>
                )}
              </div>
            </div>
